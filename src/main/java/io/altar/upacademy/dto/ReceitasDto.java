...
public class ReceitasDto extends EntityService implements Serializable {
    ...
    public String calcular() {
        long id = ((Number) em.createNativeQuery("SELECT id FROM Receita R " + "WHERE R.nome='" + receita.getNome() + "'").getSingleResult()).longValue();
        int cal = ((Number) em.createNativeQuery("SELECT sum(calorias*Quantidade/100) FROM Ingrediente I " + "INNER JOIN Receita_Ingrediente RI ON I.id = RI.ingrediente_id WHERE RI.receita_id=" + id).getSingleResult()).intValue();
        double prot = ((Number) em.createNativeQuery("SELECT sum(proteina*Quantidade/100) FROM Ingrediente I " + "INNER JOIN Receita_Ingrediente RI ON I.id = RI.ingrediente_id WHERE RI.receita_id=" + id).getSingleResult()).doubleValue();
        double hid = ((Number) em.createNativeQuery("SELECT sum(hidratos*Quantidade/100) FROM Ingrediente I " + "INNER JOIN Receita_Ingrediente RI ON I.id = RI.ingrediente_id WHERE RI.receita_id=" + id).getSingleResult()).doubleValue();
        double gord = ((Number) em.createNativeQuery("SELECT sum(gorduras*Quantidade/100) FROM Ingrediente I " + "INNER JOIN Receita_Ingrediente RI ON I.id = RI.ingrediente_id WHERE RI.receita_id=" + id).getSingleResult()).doubleValue();
        int qtd = ((Number) em.createNativeQuery("SELECT sum(Quantidade) FROM Ingrediente I " + "INNER JOIN Receita_Ingrediente RI ON I.id = RI.ingrediente_id WHERE RI.receita_id=" + id).getSingleResult()).intValue();
        int ppl;
